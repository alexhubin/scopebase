    ...
    <div className="min-h-screen overflow-hidden bg-line">
      ...
      <main>
        <section className="mt-px grid gap-px bg-line lg:grid-cols-2">
          <div className="bg-white px-5 py-14 sm:px-10 sm:py-[72px]">
          ...
          <div className="grid content-center bg-paper p-5 sm:p-10">
            <div className="border border-line bg-white shadow-[0_30px_80px_rgba(23,63,53,0.13)]">
              <div className="flex items-center justify-between border-b border-line px-6 py-[18px]"><div><p className="text-[11px] font-bold uppercase tracking-[0.1em] text-[#9a5d08]">Waiting for approval</p><h2 className="mt-1.5 text-lg font-semibold">Dental clinic website redesign</h2></div><span className="grid size-[38px] place-items-center border border-line text-xs font-bold text-sage">v1</span></div>
        ...
        <section className="mt-px grid gap-px bg-line sm:grid-cols-2 lg:grid-cols-4">
          ...
          <a href="#workflow" className="grid content-end bg-mint px-6 py-9 sm:px-10"><strong className="flex items-center gap-2 text-[15px]">See the full workflow <ArrowDown size={15} /></strong><span className="mt-1.5 text-[13px] text-[#52625d]">3 steps · 2 sides of the table</span></a>
        </section>
        ...
        <section id="product" className="mt-px grid gap-px bg-line lg:grid-cols-4">
          ...
          {problems.map(([title, description], index) => <div key={title} className="grid bg-white sm:grid-cols-[80px_1fr] lg:col-span-3"><span className="grid place-items-center border-b border-line py-5 font-display text-[15px] font-semibold text-sage sm:border-b-0 sm:border-r">0{index + 1}</span><div className="px-6 py-[26px] sm:px-10"><h3 className="font-sans text-[17px] font-bold">{title}</h3><p className="mt-2 text-[15px] leading-[1.6] text-[#52625d]">{description}</p></div></div>)}
        </section>
        ...
        <section id="workflow" className="mt-px grid gap-px bg-line lg:grid-cols-4">
          ...
          <div className="grid content-end bg-white p-6 sm:p-10 lg:col-span-3"><p className="max-w-[640px] text-base leading-[1.65] text-[#52625d]">Everything you publish has a matching client page — no accounts, no attachments, just a secure link. Every decision is recorded against the exact version shown.</p></div>
        ...
        <section className="mt-px grid gap-px bg-line sm:grid-cols-2 lg:grid-cols-4">
          ...
          {audiences.map(([eyebrow, title, description], index) => <article key={eyebrow} className={`p-8 sm:p-10 ${index === 2 ? "bg-mint" : "bg-white"}`}><p className="text-[11px] font-bold uppercase tracking-[0.1em] text-sage">{eyebrow}</p><h3 className="mt-3 text-[19px] font-semibold">{title}</h3><p className="mt-2.5 text-[14.5px] leading-[1.65] text-[#52625d]">{description}</p></article>)}
        </section>
        ...
        <section id="pricing" className="mt-px grid gap-px bg-line sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="grid content-end bg-paper p-8 text-[14.5px] leading-[1.65] text-[#52625d] sm:p-10"><p>All plans include unlimited clients, secure client links, and EU data hosting.</p><p className="mt-3.5">No credit card required · Cancel anytime.</p></div>
        </section>
        ...
        <section id="faq" className="mt-px grid gap-px bg-line lg:grid-cols-4">
          ...
          {faqs.map(([question, answer]) => <article key={question} className="bg-white px-6 py-[26px] sm:px-10 lg:col-span-3"><h3 className="font-sans text-base font-bold">{question}</h3><p className="mt-2 text-[15px] leading-[1.6] text-[#52625d]">{answer}</p></article>)}
        </section>
...
function Stat({ value, children }: { value: string; children: React.ReactNode }) {
  return <article className="bg-white px-6 py-9 sm:px-10"><strong className="block font-display text-4xl font-semibold tracking-[-0.02em] text-coral">{value}</strong><p className="mt-2 text-sm leading-[1.6] text-[#52625d]">{children}</p></article>;
}

function SectionIntro({ eyebrow, title, className = "" }: { eyebrow: string; title: string; className?: string }) {
  return <div className={`bg-white p-8 sm:p-10 ${className}`}><p className="text-xs font-bold uppercase tracking-[0.12em] text-coral">{eyebrow}</p><h2 className="mt-[18px] text-[32px] font-semibold leading-[1.15] tracking-[-0.02em]">{title}</h2></div>;
}

function WorkflowRow({ index, title, description }: { index: number; title: string; description: string }) {
  return <><article className="grid content-center justify-items-start bg-white p-8 sm:p-10 lg:col-span-2"><span className="grid size-10 place-items-center border border-line font-display text-[15px] font-semibold text-coral">0{index + 1}</span><span className="mt-[22px] text-[11px] font-bold uppercase tracking-[0.1em] text-sage">You</span><h3 className="mt-2 text-[21px] font-semibold">{title}</h3><p className="mt-3 max-w-[480px] text-[15px] leading-[1.65] text-[#52625d]">{description}</p></article><article className="grid content-center bg-paper p-8 sm:p-10 lg:col-span-2"><p className="text-[11px] font-bold uppercase tracking-[0.1em] text-coral">Your client sees</p><ClientCard index={index} /></article></>;
}
