...
import { ArrowRight, CircleCheckBig, Clock3, FileQuestion, FolderKanban } from "lucide-react";
...
export function DashboardPage() {
  ...
  const metrics = [
    [FolderKanban, "Active projects", summary.data?.active_projects ?? 0, "All work currently moving"],
    [FileQuestion, "Waiting for brief", summary.data?.briefs_waiting ?? 0, "Clients with the next action"],
    [Clock3, "Scope approvals", summary.data?.scopes_waiting ?? 0, "Published versions under review"],
    [CircleCheckBig, "Change requests", summary.data?.pending_change_requests ?? 0, "Additional work awaiting decision"],
  ] as const;

  return (
    <div className="animate-rise">
      <PageHeader eyebrow="Workspace pulse" title="Keep every project moving" description="The next client action and the latest agreement are visible at a glance." action={<Link to="/projects" className="focus-ring inline-flex min-h-10 items-center gap-2 rounded-lg bg-forest px-4 text-sm font-bold text-white">View all projects <ArrowRight size={16} /></Link>} />
      <div className="grid gap-px border border-line bg-line sm:grid-cols-2 xl:grid-cols-4">
        {metrics.map(([Icon, label, value, description]) => (
          <article key={label} className="bg-white p-5 sm:p-6">
            <div className="flex items-start justify-between"><p className="text-sm font-bold text-sage">{label}</p><Icon size={19} className="text-coral" /></div>
            <p className="mt-6 text-4xl font-extrabold tracking-[-0.04em] text-ink">{value}</p>
            <p className="mt-2 text-xs leading-5 text-sage">{description}</p>
          </article>
        ))}
      </div>
      ...
      <div className="mt-7 grid gap-7 xl:grid-cols-[1.3fr_0.7fr]">
        <Panel>
          <div className="flex items-center justify-between border-b border-line px-5 py-4 sm:px-6">
            <h2 className="font-extrabold text-ink">Recent projects</h2>
            ...
            <div className="divide-y divide-line">
              {projects.data.items.map((project) => (
                <Link key={project.id} to="/projects/$projectId" params={{ projectId: project.id }} className="focus-ring grid gap-3 px-5 py-5 transition hover:bg-paper sm:grid-cols-[1fr_auto_auto] sm:items-center sm:px-6">
                  <div className="min-w-0"><p className="truncate font-extrabold text-ink">{project.name}</p><p className="mt-1 text-xs text-sage">{project.client_name}</p></div>
                  ...
                  <p className="text-sm font-bold text-ink sm:w-24 sm:text-right">{money(project.base_price, project.currency)}</p>
                </Link>
              ))}
            </div>
        ...
        <Panel>
          <div className="border-b border-line px-5 py-4 sm:px-6"><h2 className="font-extrabold text-ink">Recent activity</h2></div>
