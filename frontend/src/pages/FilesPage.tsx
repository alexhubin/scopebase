...
export function FilesPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div><h2 className="text-2xl font-extrabold tracking-[-0.03em]">Files</h2><p className="mt-2 text-sm leading-6 text-sage">Keep client source material and project documents attached to the right project.</p></div><div><input ref={inputRef} className="sr-only" type="file" accept={acceptedTypes} onChange={(event) => { const file = event.target.files?.[0]; if (file) upload.mutate(file); }} /><Button onClick={() => inputRef.current?.click()} disabled={upload.isPending}><Upload size={16} /> {upload.isPending ? "Uploading" : "Upload file"}</Button></div></div>
      {files.data?.length ? <Panel><div className="divide-y divide-line">{files.data.map((item) => <div key={item.id} className="flex items-center gap-4 px-5 py-4"><span className="grid size-10 shrink-0 place-items-center rounded-full bg-mint text-forest"><File size={18} /></span><div className="min-w-0 flex-1"><p className="truncate text-sm font-extrabold">{item.original_filename}</p><p className="mt-1 text-xs text-sage">{formatBytes(item.size)} · {shortDate(item.created_at)}</p></div><Button className="min-h-9 px-2" variant="ghost" aria-label={`Download ${item.original_filename}`} onClick={() => void openDownload(item.id)}><Download size={16} /></Button><Button className="min-h-9 px-2 text-red-700" variant="ghost" aria-label={`Delete ${item.original_filename}`} onClick={() => deleteFile.mutate(item.id)} disabled={deleteFile.isPending}><Trash2 size={16} /></Button></div>)}</div></Panel> : <EmptyState title="No files yet" description="Upload a project document, or invite the client to attach files while completing the brief." action={<Button variant="secondary" onClick={() => inputRef.current?.click()}>Choose file</Button>} />}
