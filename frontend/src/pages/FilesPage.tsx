...
export function FilesPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {files.data?.length ? <Panel><div className="divide-y divide-line">{files.data.map((item) => <div key={item.id} className="grid grid-cols-[48px_1fr] items-center gap-4 px-5 py-4 sm:grid-cols-[48px_1fr_auto_auto] sm:px-6"><span className="grid size-11 shrink-0 place-items-center bg-mint text-[11px] font-bold text-forest"><File size={18} /></span><div className="min-w-0"><p className="truncate text-[14.5px] font-bold">{item.original_filename}</p><p className="mt-1 text-[13px] text-sage">{formatBytes(item.size)} · {shortDate(item.created_at)}</p></div><div className="col-span-2 flex justify-end gap-2 sm:col-span-1 sm:contents"><Button className="min-h-9" variant="secondary" aria-label={`Download ${item.original_filename}`} onClick={() => void openDownload(item.id)}><Download size={15} /> Download</Button><Button className="min-h-9 px-2 text-red-700" variant="ghost" aria-label={`Delete ${item.original_filename}`} onClick={() => deleteFile.mutate(item.id)} disabled={deleteFile.isPending}><Trash2 size={15} /> <span className="hidden sm:inline">Delete</span></Button></div></div>)}</div></Panel> : <EmptyState title="No files yet" description="Upload a project document, or invite the client to attach files while completing the brief." action={<Button variant="secondary" onClick={() => inputRef.current?.click()}>Choose file</Button>} />}
