...
export function ProjectsPage() {
  ...
  return (
    <div className="animate-rise">
      <PageHeader eyebrow="Client work" title="Projects" description="Each project holds its scope, decisions, files, and the next client action." action={<Button variant="accent" type="button" onClick={() => setOpen(true)}><Plus size={17} /> New project</Button>} />
      <div className="mb-5 grid gap-3 sm:grid-cols-[1fr_220px]">
        <Input type="search" aria-label="Search projects" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search by project or client" />
      ...
      {query.isLoading ? <Spinner label="Loading projects" /> : query.error ? <RetryPanel message={query.error.message} onRetry={() => void query.refetch()} /> : query.data?.items.length ? (
        <Panel>
          <div className="hidden grid-cols-[1.6fr_1fr_0.8fr_0.6fr] gap-[18px] border-b border-line px-6 py-3.5 text-[11px] font-bold uppercase tracking-[0.1em] text-sage md:grid"><span>Project</span><span>Status</span><span>Delivery</span><span className="text-right">Value</span></div>
          <div className="divide-y divide-line">
            {query.data.items.map((project) => (
              <Link key={project.id} to="/projects/$projectId" params={{ projectId: project.id }} className="focus-ring grid min-h-[72px] gap-3 px-5 py-3 transition hover:bg-paper md:grid-cols-[1.6fr_1fr_0.8fr_0.6fr] md:items-center md:gap-[18px] md:px-6">
                <div className="min-w-0"><p className="truncate text-[14.5px] font-bold text-ink">{project.name}</p><p className="mt-1 truncate text-[13px] text-sage">{project.client_name} · {project.client_email}</p></div>
                ...
                <p className="text-[13.5px] text-[#52625d]">{shortDate(project.target_delivery_date)}</p>
                <p className="text-[14.5px] font-bold text-ink md:text-right">{money(project.base_price, project.currency)}</p>
              </Link>
            ))}
          </div>
        </Panel>
        ...
        <Dialog.Portal>
          ...
          <Dialog.Viewport className="fixed inset-0 z-50 grid place-items-center overflow-y-auto p-4">
            <Dialog.Popup className="w-full max-w-2xl border border-line bg-paper p-6 shadow-2xl sm:p-8">
              <div className="flex items-start justify-between"><div><Dialog.Title className="font-display text-2xl font-semibold tracking-tight text-ink">New project</Dialog.Title><Dialog.Description className="mt-2 text-sm text-sage">Add the project essentials. The brief comes next.</Dialog.Description></div><Dialog.Close className="focus-ring p-2 text-sage hover:bg-mint"><X size={20} /></Dialog.Close></div>
