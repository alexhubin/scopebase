  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
        <div><h2 className="text-2xl font-extrabold tracking-[-0.03em]">Project scope</h2><p className="mt-2 text-sm leading-6 text-sage">Define what is included, publish a fixed version, and collect the client decision.</p></div>
        {canStart ? <Button onClick={beginDraft}><Plus size={16} /> {latest ? "Create new version" : "Create scope"}</Button> : null}
      </div>

      {publicLink ? <Panel className="flex flex-col gap-4 border-forest bg-mint/50 p-5 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><p className="text-sm font-extrabold">Secure review link created</p><p className="mt-1 truncate text-xs text-sage">{publicLink}</p></div><Button variant="secondary" onClick={() => void copyLink()}><Clipboard size={16} /> Copy link</Button></Panel> : null}
      ...
      {ordered.length ? (
        <Panel>
          <div className="border-b border-line px-5 py-4"><h3 className="font-extrabold">Version history</h3><p className="mt-1 text-xs text-sage">Published versions are immutable and approvals always reference one exact version.</p></div>
          <div className="divide-y divide-line">{ordered.map((scope) => <div key={scope.id} className="flex flex-col justify-between gap-4 px-5 py-4 sm:flex-row sm:items-center"><div className="flex items-start gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-full bg-mint text-forest"><FileCheck2 size={18} /></span><div><div className="flex flex-wrap items-center gap-2"><p className="font-extrabold">Version {scope.version_number} · {scope.title}</p><ScopeBadge status={scope.status} /></div><p className="mt-1 text-xs text-sage">{money(scope.price, project.data.currency)} · {shortDate(scope.delivery_date)} · created {shortDate(scope.created_at)}</p></div></div><div className="flex gap-2">{scope.status === "sent" ? <Button variant="secondary" onClick={() => publish.mutate(scope.id)} disabled={publish.isPending}><Send size={15} /> Send new link</Button> : null}{scope.status === "approved" ? <Button variant="secondary" onClick={() => void download(`/projects/${projectId}/scopes/${scope.id}/pdf`, `${project.data.slug}-scope-v${scope.version_number}.pdf`)}><Download size={15} /> PDF</Button> : null}</div></div>)}</div>
        </Panel>
...
function ScopeForm({ draft, setDraft, saving, onSave, onPublish, publishing }: { draft: ScopeDraft; setDraft: (draft: ScopeDraft) => void; saving: boolean; onSave: () => void; onPublish?: () => void; publishing: boolean }) {
  ...
  return (
    <Panel className="p-5 sm:p-6">
      <div className="flex flex-col justify-between gap-3 border-b border-line pb-5 sm:flex-row sm:items-center"><div><h3 className="font-extrabold">Draft scope</h3><p className="mt-1 text-xs text-sage">Save freely until you publish this version.</p></div><div className="flex gap-2"><Button variant="secondary" onClick={onSave} disabled={saving}>Save draft</Button>{onPublish ? <Button onClick={onPublish} disabled={publishing}><Send size={16} /> Publish version</Button> : null}</div></div>
